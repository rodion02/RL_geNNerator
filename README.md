...
#### Планы развития проекта как стартапа
1) Наши пользователи - ML-разработчики, интузиасты в сфере ИИ.
2) Многие организации тратят немало своего времени на простые задачи, легко решаемые нейронными сетями. С помощью нашего сервиса можно сэкономить время и деньги при работе над такими задачами.
3) Сценарий использования нашего продукта - у пользователя есть датасет, и ему необходимо создать нейронную сеть для решения поставленной задачи. Он заходит на наш сайт, авторизуется и переходит на страницу конфигурации обучения (на странице конфигурации можно выбирать различные параметры и загрузить датасет). После выбора всех необходимых параметров пользователь жмет кнопку "начать подбор" и ждёт результата (его перекидывает на другую страницу, на которую в ходе обучения выводятся метрики и анимация построения структуры нейросети). По окончании подбора пользователь может либо скачать файл с весами обученной нейросети, либо продолжить подбор с уже заданными параметрами, либо вернуться к выбору параметров.
4) Требования к нашему продукту - птимальное соотношение между быстродействием, объемом занимаемой памяти и точностью, полученной в ходе подбора нейронной сети (приоритеты выбираются самим пользователем).
5) Есть возможность сделать дешевый прототип для проверки предположений, но некоторый функционал будет не реализован (некая бэта-версия).
